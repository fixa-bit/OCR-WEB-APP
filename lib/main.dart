...
import 'package:urdu_ocr/constants/strings.dart';
...
import 'package:universal_html/html.dart' as htmlx;
...
import '/custom_image_crop.dart';
import '/result_screen.dart';


void main() {
   UsersBinding().dependencies();
   newio.HttpOverrides.global = MyHttpOverrides();
 ...
 @override
  ...
  //final Rx<TextEditingController> textcontroller.obs;
   ...
   final Rx<TextEditingController> pageimagescreen = TextEditingController().obs;
     ...
     final Rx<TextEditingController>  _widthController = TextEditingController().obs;
  final Rx<TextEditingController>  _heightController = TextEditingController().obs;
  final Rx<TextEditingController>  _radiusController = TextEditingController().obs;
  ...
  final RxBool docropping=false.obs;



  final Rx<Uint8List> croppingimage = Uint8List.fromList([0]).obs;


  RxInt isUrdu = 1.obs;//1 for urdu 0 for english
  final Rx<CustomImageCropController> controllercropper = CustomImageCropController().obs;


  //late CustomImageCropController controller;
  final Rx<CustomCropShape> _currentShape = CustomCropShape.Square.obs;
  final Rx<CustomImageFit> _imageFit = CustomImageFit.fillCropHeight.obs;
  ...
  // final TextEditingController _radiusController = TextEditingController();
    ...
    final RxDouble _width = 16.0.obs;
      final RxDouble _height = 9.0.obs;
        final RxDouble _radius = 4.0.obs;
...
class MyHttpOverrides extends newio.HttpOverrides{
  @override
  newio.HttpClient createHttpClient(newio.SecurityContext? context){
    return super.createHttpClient(context)
      ..badCertificateCallback = (newio.X509Certificate cert, String host, int port)=> true;
  }
}
...
اگر آپ کے پاس اس ایپ کے بارے میں کوئی سوال یا تجویز ہے، تو برائے مہربانی اپنی قیمتی آراء یہاں شیئر کریں۔

''',
  '''If you have query or suggestion regarding this app, kindly share your valuable feedback here
...
براہ کرم یقینی بنائیں کہ اپ لوڈ کردہ تصویر سیدھی ہے اور متن پڑھنے کے قابل ہے۔
'''
,'''
The Application takes two forms of input i.e., image and pdf document.
...
This works best with Jameel Noori Nastaleeq and Faiz Lahori Nastaleeq fonts.
Please make sure that uploaded picture is straight and text is readable.
'''
...
PopupMenuButton<int>(
          ...
          onPressed: () => downloadandroidapk("assets/assets/urdu_ocr.apk"),//Text('Android APK')),
          child: Text("Android Mobile APK"),
        ),
      ),
               PopupMenuItem<int>(
                  value: 2, child:  TextButton(
                   // onPressed: () => downloadandroidapk("/assets/assets/xxxx.jpg"),//Text('Android APK')),
        onPressed: () => downloaddesktopapk("assets/assets/UOCR_windows_beta.zip"),//Text('Android APK')),
          child: Text("Windows Setup"),
        ),//Text('Desktop Zip')
                  ),
               PopupMenuItem<int>(
                  value: 3, child:  TextButton(
                   //onPressed: () => downloadandroidimg("assets/assets/xxxx.jpg"),//Text('Android APK')),
          onPressed: () => downloadpackages("assets/assets/UOCR_linux_beta.zip"),//Text('Android APK')),
          child: Text("Linux Setup"),
        ),//Text('Packages')
                  ),
 ...
 controller.isUrdu.value==1?
  ...
  text: TextSpan(
    ...
    children: <TextSpan>[
      ...
      TextSpan(
              text: 'گوگل فارم', style: const TextStyle(color:Colors.blue,fontWeight: FontWeight.bold),
              recognizer: TapGestureRecognizer()
                ..onTap = () => launch("https://docs.google.com/forms/d/e/1FAIpQLSf0lfFf9lj3mQ83sZv3h1L2xeaksuEWmJrx5DCRHgyFoDieUg/viewform?usp=sf_link")),
...
RichText(
  text: TextSpan(
    ...
    children: <TextSpan>[
      ...
      TextSpan(
              text: 'Google Form', style: const TextStyle(color:Colors.blue,fontWeight: FontWeight.bold),
              recognizer: TapGestureRecognizer()
                ..onTap = () => launch("https://docs.google.com/forms/d/e/1FAIpQLSf0lfFf9lj3mQ83sZv3h1L2xeaksuEWmJrx5DCRHgyFoDieUg/viewform?usp=sf_link")),
...
//       ),//endbody
    ...
    downloadandroidimg(url) {
  AnchorElement anchorElement = new AnchorElement(href: url);
  anchorElement.download = "xxx.jpg";
  anchorElement.click();
}
  downloadandroidapk(url) {
  AnchorElement anchorElement = new AnchorElement(href: url);
  anchorElement.download = "urdu_ocr.apk";
  anchorElement.click();
}
  downloaddesktopapk(url) {
  AnchorElement anchorElement = new AnchorElement(href: url);
  anchorElement.download = "UOCR_windows_setup.zip";
  anchorElement.click();
}
  downloadpackages(url) {
  AnchorElement anchorElement = new AnchorElement(href: url);
  anchorElement.download = "UOCR_linux_setup.zip";
  anchorElement.click();
}
...
class Editimage extends GetView<UsersController> {
  ...
  void _changeCropShape(CustomCropShape newShape) {
    //setState(() {
      controller._currentShape.value = newShape;
    //});
  }

  void _changeImageFit(CustomImageFit imageFit) {
   // setState(() {
      controller._imageFit.value = imageFit;
    //});
  }

  void _updateRatio() {
    //setState(() {
      if (controller._widthController.value.text.isNotEmpty) {
        controller._width.value = double.tryParse(controller._widthController.value.text) ?? 16;
      }
      if (controller._heightController.value.text.isNotEmpty) {
        controller._height.value = double.tryParse(controller._heightController.value.text) ?? 9;
      }
      if (controller._radiusController.value.text.isNotEmpty) {
        controller._radius.value = double.tryParse(controller._radiusController.value.text) ?? 4;
      }
    //});
...
Future<Uint8List> readbytes5() async {
 ...
 //else {
   if(controller.filter.value == "crop"){
     controller.croppingimage.value=controller.imagevalue.value;
    //  print("cropped");
    //  resetSlider();
    ...
    //  Uint8List jpgx = Uint8List.fromList(im.encodeJpg(brightenedImage!));   
    //        controller.imagevalue.value = jpgx;
    //  updateImage();
      ...
      if (controller.filter.value == "brightness") {
        //controller.docropping.value==false;
...
resetSlider() {
 ...
 controller.isUrdu.value==1?
            ...
            :
               ...
               // Text("Edited Image"), 
               Obx(() {return
                             controller.loadingimage.value==true && controller.imagevalue.value.length>1
                             ?
              Stack(
          ...
          FittedBox(
            ...
            fit: BoxFit.contain,
                ...
                LoadingAnimationWidget.flickr(
                    leftDotColor: Color.fromARGB(255, 64, 18, 117),
     ...
     width: (MediaQuery.of(context).size.width)/4,
      ...
      ?
                ...
                //width: 370,color:Colors.black);
                                              ...
                                              :
                                                      FittedBox(
            child: Image.asset('/nofile.png',color: Color.fromRGBO(255, 255, 255, 0.1),
            colorBlendMode: BlendMode.modulate),
            fit: BoxFit.fitHeight,
          );
    ...
    child:Container(
          ...
          FittedBox(
            ...
            child:
              Obx(() {return
            controller.imagevalue.value.length<=1 ?
            //Image.asset('assets/download2.jpg',color: Color.fromRGBO(255, 255, 255, 0.01),
            Image.asset('/nofile.png',color: Color.fromRGBO(255, 255, 255, 0.3),
            colorBlendMode: BlendMode.modulate)
            :
             ...
             Image.memory(controller.imagevalueorg.value,
            colorBlendMode: BlendMode.modulate)
            ;}),
 ...
 Text(restore[0], textDirection: TextDirection.rtl, style: TextStyle(color:Colors.white,fontWeight: FontWeight.bold, fontFamily:
             ...
             onPressed: () async {
               controller.loadingimage.value=true;
               print("loading image in progres true");
         ...
         im.Image? newImagexxx_ = (im.decodeImage(controller.imagevalueorg.value)!).clone();
          ...
          Uint8List mBytes =  Uint8List.fromList(im.encodeJpg(newImagexxx_!));
                               ...
                               controller.thumbnailimagex.value =im.decodeImage(controller.imagevalueorg.value)!.clone();
                               controller.loadingimage.value=false;
                               print("loading image in progres now false");
                               // brightenedImage = controller.thumbnailimagex.value;
...
ElevatedButton(
 ...
 Text(done[0], textDirection: TextDirection.rtl, style: TextStyle(color:Colors.white,fontWeight: FontWeight.bold, fontFamily:
                           'Nastaleeq_khasheeda',letterSpacing:2.0, fontSize: 14.0))
                           :
        ...
        onChanged: (double newValue) {
          if (controller.filter.value == 'brightness'&& controller.imagevalue.value.length>1  ) {
        ...
        label: 'Set value',
        onChanged: (double newValue) {
          if (controller.filter.value == 'contrast'&& controller.imagevalue.value.length>1  ) {
     ...
     Container(height: 30,
        ...
        label: 'Set value',
        onChanged: (double newValue) {
          if (controller.filter.value == 'saturation'&& controller.imagevalue.value.length>1  ) {
        ...
        label: 'Set value',
        onChanged: (double newValue) {
          if (controller.filter.value == 'sharpness'&& controller.imagevalue.value.length>1  ) {
        ...
        child: 
            ...
            onPressed: () {
                   controller._exposurexold.value=0;
        ...
        label: 'Set value',
        onChanged: (double newValue) {
          if (controller.filter.value == 'exposure'&& controller.imagevalue.value.length>1  ) {
                        ...
                        child:const Row(
                          ...
                          children: <Widget>[
                           // SizedBox(width: 50,height: 60,),
                            Icon(Icons.crop),
                           // SizedBox(width: 20,height: 0,),
                           Text('Crop and Rotate',textDirection: TextDirection.rtl, style: TextStyle( color:Color.fromARGB(255, 139, 139, 139),fontWeight: FontWeight.bold, fontFamily:
                           'Nastaleeq_khasheeda',letterSpacing:2.0, fontSize: 15.0)),
                          ],
                        ),) ,
             onPressed: () async {

//  controller.croppingimage.value = Uint8List.fromList([0]);
  ...
  //   controller._width.value = 16.0;
  //     controller._height.value = 9.0;
  //       controller._radius.value = 4.0;




               controller.docropping.value=true;
               
              // await controller.controllercropper.value.reset;
              // await Future.delayed(const Duration(seconds: 2));
               
              controller.croppingimage.value=await controller.imagevalue.value;
                controller.controllercropper.value.reset();
              ...
              style: ElevatedButton.styleFrom(
                ...
                controller.docropping.value==true? Color.fromARGB(255, 24, 136, 20):Color.fromARGB(155, 75, 75, 75), //background color of button
...
//                            // SizedBox(width: 20,height: 0,),
//                            Text('reset', textDirection: TextDirection.rtl, style: TextStyle(fontWeight: FontWeight.bold, fontFamily:
//                            'Nastaleeq_khasheeda',letterSpacing:2.0, fontSize: 15.0)),
//                           ],
//                         ),) ,
//              onPressed: () async {
//               await controller.controllercropper.value.reset;
//                controller.croppingimage.value=controller.imagevalue.value;
//                final xx=controller.imagevalue.value;

//                await Future.delayed(const Duration(seconds: 2));
//                await controller.controllercropper.value.reset;
//                controller.croppingimage.value=xx;
...
Column(
        children: [
          ...
          //     Obx(() {return Image.memory(
          //       controller.croppingimage.value);}))
          // :
...
Obx(() {return
          ...
          //          IconButton(
          //   icon: Icon(Icons.refresh,color: const Color.fromARGB(255, 228, 228, 228),),
          //   onPressed: () {
          //     controller.controllercropper.value.reset();
          //      //Get.rootDelegate.toNamed(Routes.HOME);
          ...
          // ),
              ...
              IconButton(
                  ...
                  onPressed: () =>
                  // controller.docropping.value==true?
                  // controller.controllercropper.value.addTransition(CropImageData(scale: 1))
                  // :
                  

                      controller.controllercropper.value.addTransition(CropImageData(scale: 1.33))),
              ...
              IconButton(
                ...
                //  controller.controllercropper.value.addTransition(CropImageData(scale: 1))
                //   :
                      controller.controllercropper.value.addTransition(CropImageData(scale: 0.75))),
              ...
              IconButton(
                  ...
                  onPressed: () =>
                  // controller.docropping.value==true?
                  // controller.controllercropper.value.addTransition(CropImageData(angle:0))
                  // :
                      controller.controllercropper.value.addTransition(CropImageData(angle: -pi / 4))),
              IconButton(
              ...
              IconButton(
                ...
                onPressed: () async {
                  final image = await controller.controllercropper.value.onCropImagex();
                  //controller.imagevalue.value=await image!;
                 // controller.croppingimage.value=await image!;

                  controller.filter.value = "crop";
                  controller.imagevalue.value=await image!;
          ...
          controller.thumbnailimagex.value =await im.decodeImage(controller.imagevalue.value)!.clone();
           //await Future.delayed(const Duration(seconds: 2));
          ...
          if (controller._currentShape.value == CustomCropShape.Ratio) ...[
            SizedBox(
              child: Row(
                children: [
                  Expanded(
                    child: TextField(
                      controller: controller._widthController.value,
                  ...
                  Expanded(
                    child: TextField(
                      controller: controller._heightController.value,
                      keyboardType: TextInputType.number,
                  ...
                  Expanded(
                    child: TextField(
                      controller: controller._radiusController.value,
                      keyboardType: TextInputType.number,
           ...
           Container(
            ...
            child:
              Obx(() {return 
            controller.croppingimage.value.length<=1?
            Container()
            :
              ...
              CustomImageCrop(
              cropController: controller.controllercropper.value,
              ...
              shape: controller._currentShape.value,
              ratio: controller._currentShape.value == CustomCropShape.Ratio
                  ? Ratio(width: controller._width.value, height: controller._height.value)
                  : null,
              canRotate: false,
              canMove: true,
              ...
              canScale: true,
              borderRadius:
                  controller._currentShape.value == CustomCropShape.Ratio ? controller._radius.value : 0,
              customProgressIndicator: const CupertinoActivityIndicator(),
              imageFit: controller._imageFit.value,
              pathPaint: Paint()
                ..color = Colors.red
                ..strokeWidth = 4.0
...
cropimage(context) async {
  if (controller.imagevalue.value != null) {
      ImageCropping.cropImage(
          context: context,
          imageBytes: controller.imagevalue.value!,
          onImageDoneListener: (data) {
    
                controller.imagevalue.value = data;
...
class imageclass extends GetView<UsersController> {
 ...
 PlatformFile? objFile;
  ...
  // String backup_uri='http://192.168.2.48:8000/extract_text';
  // String primary_uri='http://192.168.2.48:5000/extract_text'; 
  String backup_uri='https://192.168.2.139/extract_text';
  String primary_uri='https://192.168.2.139/extract_text'; 
  // String backup_uri='https://10.2.38.84/extract_text';
  // String primary_uri='https://10.2.38.83/extract_text'; 
  //String backup_uri='https://192.168.2.203:5127/extract_text';
  // String primary_uri='https://192.168.2.203:5126/extract_text'; 
  // String backup_uri='https://192.168.2.203:5127/extract_text'; 
...
redirect_toEditscreen() async {
    ...
    controller._exposurex.value = 0.0;
    controller._saturationx.value = 1;
      var resultxx = await FlutterImageCompress.compressWithList(
      controller.imagevalue.value,
      minHeight: 1920,
      minWidth: 1920,
      quality: 90,
      ...
      var xx = await FlutterImageCompress.compressWithList(
      controller.imagevalue.value,
      minHeight: 1920,
      minWidth: 1920,
      quality: 90,
    ...
    controller.imagevalueorg.value=xx;
           
           controller.thumbnailimagex.value =await   im.decodeImage(resultxx);
    ...
    return             showDialog(
      ...
      // controller.thumbnailimagex.value =  im.decodeImage(controller.imagevalue.value);
...
printpdf() async {
  ...
  apiselect(context) async {
    ...
    "POST",
    //Uri.parse("http://127.0.0.1:8000/extract_text"), // Update the endpoint for image upload
    // Uri.parse("http://192.168.2.202:8000/extract_text"), // Update the endpoint for image upload
         Uri.parse(backup_uri), // Update the endpoint for image upload
  );
  request.files.add(
    http.MultipartFile(
      'image',
  ...
  if (response.statusCode == 200) {
    ...
    var responseBody = await response.stream.bytesToString();
     ...
     controller._extractedTextimage.value = 'An unexpected error occurred: $e';
      ...
      //         builder: (context) => AlertDialog(
      //           //title: Text('Alert'),
      //           content:  Text('Link is currently down. Please try later.',textDirection: TextDirection.ltr,
      //         ),
  ...
  sanitize_cookies(){
   ...
   var cookies = htmlx.document.cookie!.split(';');
    ...
    for (var cookie in cookies) {
      ...
      // Sanitize cookies as needed
      if (name != '10.2.38.83'||name != '10.2.38.84'){
        // Modify or remove sensitive cookie
        htmlx.document.cookie = '$name=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;';
      }
    }
  }
...
print("image not null");
  ...
  final request = http.MultipartRequest(
    ...
    //Uri.parse("http://127.0.0.1:8000/extract_text"), // Update the endpoint for image upload
     ...
     Uri.parse(primary_uri), // Update the endpoint for image upload
        //  Uri.parse("http://10.2.38.83:8000/extract_text"), // Update the endpoint for image upload
  );
  ...
  http.Response responsex = await http.Response.fromStream(await request.send());
  print("Result: ${responsex.statusCode}");
  print(responsex.body);
  //print('filename: $platformimage!.name');
  print("executed");
  sanitize_cookies();
  ...
  //http.Response _urlResponse =  await http.get(Uri.parse("http://192.168.2.48:8000/extract_text"));

  if (responsex.statusCode == 200) {
    print('here');
      //var responseBody = await responsex.stream.bytesToString();


    
      controller._extractedTextimage.value = responsex.body;
    ...
    //"http://10.2.38.83:8000/extract_text"
    //192.168.2.191:5000
    //Handle error
    print("next called");
  ...
  apiselect(context);
  // Perform some actions or show an error message
}
}
  @override
  Widget build(BuildContext context) {
 ...
 controller.isUrdu.value==1?
  ...
  Text(
          ...
          Container(
                  height: (MediaQuery.of(context).size.height)/2,
                  width: (MediaQuery.of(context).size.width)/3,
                     color:const Color.fromARGB(200, 230, 230, 230),
                  child:
          SizedBox(
                    child: Center(
          ...
          //         )
                  :
                  controller.loadingimage.value==false &&  controller.imagevalue.value.length > 1?
...
//  Rx<TextEditingController> textController = TextEditingController().obs;
  ...
  // String backup_uri='http://192.168.2.48:8000/extract_pdf';
  // String primary_uri='http://192.168.2.48:5000/extract_pdf'; 
  // String backup_uri='https://192.168.1.104/extract_pdf';
  // String primary_uri='https://192.168.1.104/extract_pdf'; 
  String backup_uri='https://192.168.2.139/extract_pdf';
  String primary_uri='https://192.168.2.139/extract_pdf'; 
  // String backup_uri='https://10.2.38.84/extract_pdf';
  // String primary_uri='https://10.2.38.83/extract_pdf';

  // String primary_uri='https://192.168.2.203:5126/extract_pdf'; 
  // String backup_uri='https://192.168.2.203:5127/extract_pdf';
  //192.168.1.104
...
return data.buffer.asUint8List(data.offsetInBytes, data.lengthInBytes);
  ...
  void chooseFileUsingFilePicker(context) async {
    ...
    print(sizeInMb);
    if (sizeInMb > 7){
      print("File is greater thn 7 MB");
        // This file is Longer the
    }
     else{
      print("File is less thn 7 MB");
        // This file is Longer the
    }
    //var pdfBytes=objFile!.bytes!;
    if (sizeInMb>7){
    return             showDialog(
              ...
              builder: (context) => AlertDialog(
                //title: Text('Alert'),
                content:  Text('فائل کا سائز 7 MB سے زیادہ نہیں ہو سکتا۔',textDirection: TextDirection.rtl,
              ),
  ...
  final request = http.MultipartRequest(
    "POST",
    Uri.parse(backup_uri),
    //Uri.parse("http://127.0.0.1:8000/extract_pdf"),
  );
 ...
 decoration:BoxDecoration(
  ...
  borderRadius: const BorderRadius.only(
    ...
    bottomRight: Radius.circular(10.0),
                 ...
                 Row(
                  ...
                  Text("فائل کا سائز 7 ایم بی سے زیادہ نہیں ہونا چاہئے۔",textDirection: TextDirection.rtl,style: TextStyle(
                  ...
                  Text("File Size Should not Exceed 7 MB  ",textDirection: TextDirection.ltr, style: TextStyle_english.copyWith(color: Color.fromARGB(155, 210, 210, 210),),)
  ...
  // }
    ...
    downloadtext()async {
      controller.downloading.value = true;
      //Uint8List x=await _generatePdf(PdfPageFormat.a4, controller._extractedTextpdf.value);
      print(controller._extractedTextpdf.value);
      print("printed");
  ...
  downloadpdfAsImage(mypagenum,context)async {
    ...
    print("printed pg num");
              ...
              builder: (context) => AlertDialog(
                ...
                final image = page.asImage();
                //final bytes = await image.readAsBytes();
                Uint8List uint8List =Uint8List.fromList(im.encodeJpg(image!));   
                //    // Uint8List jpgx = Uint8List.fromList(im.encodeJpg(image!));   
...
class Login extends GetView<UsersController> {
  ...
  Widget build(BuildContext context) {
    ...
    Scaffold(
      appBar: AppBar(
        title: Text('Custom Crop & Rotate'),
        systemOverlayStyle: SystemUiOverlayStyle.dark,
      ),
      body: Column(
        children: [
          Expanded(
            child: Obx(() {return CustomImageCrop(
              cropController: controller.controllercropper.value,
              // image: const AssetImage('assets/test.png'), // Any Imageprovider will work, try with a NetworkImage for example...
              image:  MemoryImage(
                 controller.imagevalue.value), 
          ...
          Row(
            ...
            children: [
              IconButton(
                  icon: const Icon(Icons.refresh), onPressed: controller.controllercropper.value.reset),
              IconButton(
                  icon: const Icon(Icons.zoom_in),
                  onPressed: () =>
                      controller.controllercropper.value.addTransition(CropImageData(scale: 1.33))),
          ...
          controller.thumbnailimagex.value =im.decodeImage(controller.imagevalue.value)!.clone();
          controller.loadingimage.value=false;
          print("loading image in progres now false");
